...
const UpdaterTools := preload("res://addons/gdUnit4/src/core/GdUnitTools.gd")
...
var _update_client :UpdaterUpdateClient
...
func setup(editor_interface :EditorInterface, update_client :UpdaterUpdateClient, download_url :String) -> void:
...
func run_update() -> void:
	...
	await update_progress("Uninstall Updater4 ... [img=24x24]%s[/img]" % spinner_icon)
	disable_gdUnit()
	if not _debug_mode:
	...
	await update_progress("Install new Updater4 version ...")
	if _debug_mode:
		copy_directory(tmp_path, "res://debug")
	else:
		copy_directory(tmp_path, "res://")

	await update_progress("New Updater version successfully installed, Restarting Godot ...")
	await get_tree().create_timer(3).timeout
	enable_gdUnit()
	hide()
...
func download_release() -> void:
	...
	var response :UpdaterUpdateClient.HttpResponse
	if _debug_mode:
		response = UpdaterUpdateClient.HttpResponse.new(200, PackedByteArray())
