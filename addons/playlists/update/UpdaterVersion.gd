...
static func parse(value :String) -> UpdaterVersion:
	var regex := RegEx.new()
	...
	return UpdaterVersion.new(major, minor, patch)


static func current() -> UpdaterVersion:
	var config = ConfigFile.new()
	config.load('addons/playlists/plugin.cfg')
