...
const Translator = preload("translate.gd")
...
func _ready() -> void:
	...
	# Wait a few seconds before checking the first time
...
func check_for_update() -> void:
	if Settings.get_user_value("check_for_updates", true):
		http_request.request(REMOTE_RELEASES_URL)

## Get the current version
func _get_version() -> String:
	var config: ConfigFile = ConfigFile.new()
	config.load(_get_plugin_path() + "/plugin.cfg")
	return config.get_value("plugin", "version")

## Get the current path of the plugin
func _get_plugin_path() -> String:
	#TODO: make this config
	return "res://addons/playlists"
...
func _on_http_request_request_completed(result: int, response_code: int, headers: PackedStringArray, body: PackedByteArray) -> void:
	...
	if versions.size() > 0:
		...
		text = Translator.translate(&"update.available").format({ version = versions[0].tag_name.substr(1) })
		show()
...
func _on_update_button_pressed() -> void:
	if needs_reload:
		...
		if editor_plugin and will_refresh:
...
func _on_download_update_panel_updated(updated_to_version: String) -> void:
	...
	needs_reload_dialog.dialog_text = Translator.translate(&"update.needs_reload")
	needs_reload_dialog.ok_button_text = Translator.translate(&"update.reload_ok_button")
	needs_reload_dialog.cancel_button_text = Translator.translate(&"update.reload_cancel_button")
	needs_reload_dialog.popup_centered()

	needs_reload = true
	text = Translator.translate(&"update.reload_project")
	apply_theme()


func _on_download_update_panel_failed() -> void:
	download_dialog.hide()
	update_failed_dialog.dialog_text = Translator.translate(&"update.failed")
